        ...
        <header className="text-gray-600 body-font">
            <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
                <a href='/' className="flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0">
                    ...
                    <span className="ml-2 text-xl">Diamond</span>
                </a>
                <nav className="md:mr-auto md:ml-4 md:py-1 md:pl-4 md:border-l md:border-gray-400 flex flex-wrap items-center text-base justify-center">
                    <button
                        className={`mr-5 hover:text-gray-900 ${selectedButton === Category.SNACKS ? 'border-b-2 border-black' : ''}`}
                        onClick={() => handleButtonClick(Category.SNACKS)}
                    >
                        {Category.SNACKS}
                    </button>
                    <button
                        className={`mr-5 hover:text-gray-900 ${selectedButton === Category.LICORES ? 'border-b-2 border-black' : ''}`}
                        onClick={() => handleButtonClick(Category.LICORES)}
                    >
                        {Category.LICORES}
                    </button>
                    <button
                        className={`mr-5 hover:text-gray-900 ${selectedButton === Category.TODO ? 'border-b-2 border-black' : ''}`}
